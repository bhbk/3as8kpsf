...
--*/
{
	...
	Info = GetDeviceDescription(Devs, DevInfo);
    devInfoListDetail.cbSize = sizeof(devInfoListDetail);
    if((!SetupDiGetDeviceInfoListDetail(Devs, &devInfoListDetail)) || (CM_Get_Device_ID_Ex(DevInfo->DevInst, devID, MAX_DEVICE_ID_LEN, 0, devInfoListDetail.RemoteMachineHandle) != CR_SUCCESS)) 
	{
        lstrcpy(devID, TEXT("?"));
        b = FALSE;
    }
    ...
    if(Info) 
	{
		rslt = (LPTSTR)malloc((lstrlenW(devID) + lstrlenW(Info) + 2) * sizeof(TCHAR));
	} 
	else 
	{
		rslt = (LPTSTR)malloc((lstrlenW(devID) + 1) * sizeof(TCHAR));
    }
	lstrcpyW(rslt, devID);
	lstrcatW(rslt, TEXT("|"));
	lstrcatW(rslt, Info);
...
--*/
{
    ...
    BOOL hasInfo = FALSE;
    devInfoListDetail.cbSize = sizeof(devInfoListDetail);
    if((!SetupDiGetDeviceInfoListDetail(Devs, &devInfoListDetail)) || (CM_Get_DevNode_Status_Ex(&status, &problem, DevInfo->DevInst, 0, devInfoListDetail.RemoteMachineHandle) != CR_SUCCESS)) 
	{
        return FALSE;
    }
