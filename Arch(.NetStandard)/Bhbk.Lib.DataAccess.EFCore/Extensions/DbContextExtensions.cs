...
namespace Bhbk.Lib.DataAccess.EFCore.Extensions
{
    ...
    {
        public static T[] SqlQuery<T>(this DbContext db, string sql, object[] parameters = null, CancellationToken cancellationToken = default) where T : class
        {
            ...
            if (typeof(T).GetProperties().Any())
            {
                ...
                {
                    return db2.Set<T>().FromSqlRaw(sql, parameters).ToArray();
                }
            }
            else
            {
                db.Database.ExecuteSqlRaw(sql, parameters, cancellationToken);
                return default;
            }
        }
